...
Manager::Manager(string name, string pwd)		//有参构造
{
	this->m_Name = name;
	this->m_Pwd = pwd;
	this->initVector();			//初始化容器 获取到所有文件中 学生、老师信息
...
void Manager::addPerson()
{
	...
	if (select == 1)
	{
		fileName = STUDENT_FILE;		//添加的是学生
		tip = "请输入学号：";
		errorTip = "学号重复，请重新输入";
	}
	else
	{
		fileName = TEACHER_FILE;		//添加的是老师	
		tip = "请输入职工编号：";
...
{
	ofstream ofs(ORDER_FILE, ios::trunc);		//ios::trunc 如果文件存在，先删除后创建
	ofs.close();
	cout << "清空成功！" << endl;
	system("pause");
	system("cls");
